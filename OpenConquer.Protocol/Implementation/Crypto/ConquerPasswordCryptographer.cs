...
namespace OpenConquer.Protocol.Implementation.Crypto
{
    public sealed class ConquerPasswordCryptographer
    {
        ...
        private static readonly byte[] ScanCodeToVirtualKey =
        [
        ...
        private static readonly byte[] VirtualKeyToScanCode =
        [
        ...
        public ConquerPasswordCryptographer(string username)
        {
            ...
            for (int i = 1; i < 0x100; i++)
            {
                ...
                {
                    if (_key[(i * 2) + 1] < _key[(j * 2) + 1])
                    {
                        (_key[i * 2], _key[j * 2]) = (_key[j * 2], _key[i * 2]);
                        (_key[(i * 2) + 1], _key[(j * 2) + 1]) = (_key[(j * 2) + 1], _key[(i * 2) + 1]);
                    }
                }
            }
        }
        ...
        public byte[] Decrypt(byte[] data, int length)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                // Convert uppercase VK to lowercase ASCII if needed
        ...
        public byte[] Encrypt(byte[] data, int length)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (ch is >= 0x61 and <= 0x7A) // lowercase
                {
                    vk = (byte)(ch - 0x20);    // to uppercase
                }

                byte scanCode = VirtualKeyToScanCode[vk];
                if (ch is >= 0x41 and <= 0x5A) // original was uppercase
                {
                    scanCode += 0x80;
                }
