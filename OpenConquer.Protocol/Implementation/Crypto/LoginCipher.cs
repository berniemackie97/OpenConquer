using OpenConquer.Protocol.Interface.Crypto;
...
namespace OpenConquer.Protocol.Implementation.Crypto
{
    ...
    /// Matches the 5518 login handshake and packet encryption algorithm.
    /// </summary>
    public sealed class LoginCipher : IPacketCipher
    {
        ...
        public void Decrypt(byte[] buffer, int length)
        {
            for (int i = 0; i < length; i++)
            {
                // 1) XOR with the constant
                buffer[i] ^= 0xAB;
                // 2) nibble‑swap
                buffer[i] = (byte)(buffer[i] >> 4 | buffer[i] << 4);
                // 3) XOR with dynamic key (CryptKey2 ^ CryptKey1)
